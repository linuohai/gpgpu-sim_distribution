...
bool grasp_chain_detector_t::on_instruction_issue(
  ...
  // Tracked warp selection: first LDG from any warp becomes tracked
  if (m_tracked_warp_id == UNSET_WARP) {
    if (op == LOAD_OP) {
      m_tracked_warp_id = warp_id;
    } else {
      return false;
    }
  }

  // Only process the tracked warp
  if (warp_id != m_tracked_warp_id) return false;
...
void grasp_chain_detector_t::on_warp_exit(unsigned warp_id) {
  if (warp_id != m_tracked_warp_id) return;
  // Clear FIFO
  ...
  m_tail = 0;
  m_count = 0;
  // No warp succession: always freeze training when tracked warp exits.
  // Rationale: if strides weren't learned from the tracked warp, those PC's
  // IMA patterns are likely one-time accesses; another warp won't learn them.
  // m_tracked_warp_id preserved (not reset) for diagnostics.
