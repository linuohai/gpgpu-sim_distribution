...
bool l2_tracer::s_enabled = false;
bool l2_tracer::s_print_bw = false;
bool l2_tracer::s_print_compute = false;
std::string l2_tracer::s_path;
std::vector<std::string> l2_tracer::s_buffers;
size_t l2_tracer::s_flush_threshold = 1u << 16;

void l2_tracer::init(bool enable, const char *path, unsigned n_sms,
  ...
  file << "cycle,sm_id,warp_id,lane_id,op,space,address,l2_status,pc";
  if (s_print_bw) {
...
void l2_tracer::emit(unsigned sid, unsigned wid, const mem_fetch *mf,
...
void l2_tracer::flush_sid(unsigned sid) {
  if (!s_enabled) return;
  if (sid >= s_buffers.size()) return;
  std::string &buffer = s_buffers[sid];
