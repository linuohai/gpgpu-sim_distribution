...
## 功能概述
为便于调试和性能分析，GPGPU-Sim 新增了一个可选的 L1 Cache 追踪器。开启后，每次有访存请求到达 L1 时，模拟器都会按照活跃线程（lane）拆分，输出包含流水周期、SM/warp 标识、lane ID、读写类型、线程原始地址、L1 访问结果以及请求 UID 的 CSV 记录。通过 `mem_fetch::m_request_uid` 可以把 L1 命中/未命中信息与合并前的线程地址对应起来。
...
## 输出格式
当追踪器启用且某次访存在 L1 Cache 上完成访问后，会为请求中所有活跃 lane 追加一行 CSV 记录：

```
cycle,sm_id,warp_id,lane_id,op,address,l1_status,uid
```
...
- `uid`：`mem_fetch::get_request_uid()`，便于跨阶段关联同一访存请求。
...
2. **L1 访问钩子**：在 `l1_cache::access` 成功获得 `cache_request_status` 后调用 `l1_tracer::emit`，将 SM/warp ID、请求 UID、访问状态以及模拟周期写入对应 SM 缓冲区。非 L1 cache 不会触发追踪。【F:src/gpgpu-sim/gpu-cache.cc†L1998-L2024】
...
- 对于同时运行多个 kernel 的场景，建议结合 `uid` 与 kernel 启停时间对记录进行二次分析，以精确定位访存热点。
